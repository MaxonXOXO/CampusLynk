param(
    [string]$Port = "9222",
    [string]$OutputFile = "scripts/migration/chat-bridge/test-output.txt"
)
...
$evalPayload = @{
    ...
    params = @{
        expression = "(() => { const m = document.querySelectorAll('.markdown'); return m.length > 0 ? m[m.length - 1].innerText : ''; })()"
        returnByValue = $true
    }
...
$lastText = $obj.result.result.value

$ws.CloseAsync([System.Net.WebSockets.WebSocketCloseStatus]::NormalClosure, "Done", [System.Threading.CancellationToken]::None).Wait()

if ($lastText) {
    $lastText | Out-File -FilePath $OutputFile -Encoding utf8 -Force
    Write-Host "SAVED_CHARS: $($lastText.Length)"
} else {
    Write-Host "EMPTY"
}
