...
class ProgramAttainmentController extends Controller
{
    ...
    {
        $role = Session::get('userRole') ?: (Auth::check() ? (Auth::user()->role ?? null) : null);
        if ($role && !in_array($role, ['HOD', 'Principal', 'Super_Admin', 'Admin', 'Chairman', 'Faculty', 'Lecturer'])) {
        ...
        return view('program_attainment.index', array_merge($data, [
            'classroom' => $classroom,
    ...
    {
        $record = ProgramAttainment::firstOrCreate(['classroom_id' => $classroomId]);
    ...
    public function printReport(Request $request, $classroomId)
    {
        ...
        return view('program_attainment.print', array_merge($data, [
            'classroom' => $classroom,
            'classroomId' => $classroomId,
            'revision' => $revision,
            'programRecord' => $programRecord
    ...
    /**
     * Internal compiler that aggregates all courses in the cohort and delegates PO calculations to AttainmentService.
     */
    public function compileProgramAttainmentData($classroomId, $classroom, $revision, $programRecord): array
    {
        ...
        foreach ($subjects as $subj) {
            ...
            // Calculate PO contribution for this course via AttainmentService
        ...
        // Direct PO attainment across all courses via AttainmentService
        ...
        // Final overall PO attainment (80% Direct + 20% Indirect) via AttainmentService
