...
function patchDashboard($filePath, $role) {
    ...
    if (strpos($content, 'id="navCourseFiles"') === false) {
            ...
            <div id="cfLevel4" class="hidden space-y-6 animate-fade-in">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex gap-2">
                        <button type="button" onclick="saveCourseFileProgress()" class="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-1.5 rounded-lg text-xs font-bold transition-premium">Save Progress</button>
                        <button type="button" onclick="generateCourseFilePDF()" class="bg-amber-600 hover:bg-amber-500 text-white px-4 py-1.5 rounded-lg text-xs font-bold transition-premium flex items-center gap-1"><span class="material-symbols-rounded text-sm">picture_as_pdf</span> Generate PDF</button>
                    </div>
                </div>
                ...
                <div class="flex bg-slate-950/60 p-1.5 rounded-xl border border-slate-800">
                    <button type="button" onclick="switchCfTab(\'A\')" id="tabBtnA" class="flex-1 py-2 rounded-lg text-xs font-bold transition-premium bg-slate-800 text-white">Section A: Planning</button>
                    <button type="button" onclick="switchCfTab(\'B\')" id="tabBtnB" class="flex-1 py-2 rounded-lg text-xs font-bold transition-premium text-slate-500 hover:text-slate-300">Section B: Materials</button>
                    <button type="button" onclick="switchCfTab(\'C\')" id="tabBtnC" class="flex-1 py-2 rounded-lg text-xs font-bold transition-premium text-slate-500 hover:text-slate-300">Section C: Assessments</button>
                    <button type="button" onclick="switchCfTab(\'D\')" id="tabBtnD" class="flex-1 py-2 rounded-lg text-xs font-bold transition-premium text-slate-500 hover:text-slate-300">Section D: Attainment</button>
                </div>
...
';
    ...
    if (strpos($content, 'id="panelCourseFiles"') === false) {
        // Insert right before the last closing div of the panel container
        $targetStr = '    </div>
    
  </main>';
        $pos = strrpos($content, $targetStr);
        if ($pos !== false) {
            $content = substr_replace($content, $panelHTML . "\n" . $targetStr, $pos, strlen($targetStr));
        } else {
             // Fallback
             $content = str_replace('</main>', $panelHTML . "\n</main>", $content);
        }
    }
    ...
    function cfSelectSemester(sIdx) {
        ...
        grid.innerHTML = selectedSemester.courses.map(c => `
            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 hover:border-amber-500/50 transition-colors cursor-pointer" onclick="cfOpenBuilder(\'${c.course_file_id}\', \'${c.subject_name.replace(/\'/g, "\\\'")}\')">
                <div class="flex justify-between items-start mb-3">
                    <div class="w-10 h-10 rounded-xl bg-amber-500/10 flex items-center justify-center border border-amber-500/20">
...
';
    ...
    if (strpos($content, '// === COURSE FILES LOGIC ===') === false) {
        $content = str_replace('</script>', $jsHTML . "\n</script>", $content);
    }

    // 4. Modify switchPanel logic
    // Add courseFiles to the panels array
    ...
    // Fix the CSS classes for active/inactive nav
    ...
    echo "Successfully cleanly patched " . basename($filePath) . "\n";
}

patchDashboard("c:\\Users\\fotonlabz\\Desktop\\Test Portal\\carmel-linx-laravel\\resources\\views\\lecturer_dashboard.blade.php", "lecturer");
patchDashboard("c:\\Users\\fotonlabz\\Desktop\\Test Portal\\carmel-linx-laravel\\resources\\views\\demonstrator_dashboard.blade.php", "demonstrator");
