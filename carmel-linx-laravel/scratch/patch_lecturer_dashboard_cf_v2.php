...
// We need to replace the entire #panelCourseFiles div and the associated JS block.
// First, extract the old HTML and JS to remove them, then append the new ones.

// It's safer to use regex or string manipulation if we know the exact boundaries.
$startHtml = '      <!-- PANEL: COURSE FILES -->';
$endHtml = '      </div> <!-- End Panel Container -->'; // Or just up to the <script> tag.
// Actually, it's easier to find the exact boundaries.
$htmlPos1 = strpos($content, '      <!-- PANEL: COURSE FILES -->');
$htmlPos2 = strpos($content, '    </div>', $htmlPos1); 
// Wait, the panel has nested divs. It's better to just regex it out or do it carefully.
// Let's just find the start of the script:
$scriptStart = strpos($content, '// COURSE FILES LOGIC');
$scriptEnd = strpos($content, '// End Course Files Logic');

if ($htmlPos1 !== false && $scriptStart !== false) {
    // We'll just remove the whole chunk from <!-- PANEL: COURSE FILES --> up to </main>
    // And remove the script block.
...
';
    ...
    if (strpos($content, $newHtml) === false) {
       // regex failed, try a manual split
       $htmlPos1 = strpos($content, '<!-- PANEL: COURSE FILES -->');
       $htmlPos2 = strpos($content, '<!-- PANEL: VIRTUAL CLASSROOM -->'); // Assuming it is before this
       if ($htmlPos1 !== false && $htmlPos2 !== false) {
           $content = substr_replace($content, $newHtml . "\n      ", $htmlPos1, $htmlPos2 - $htmlPos1);
       } else {
           $htmlPos2 = strpos($content, '</main>');
           $content = substr_replace($content, $newHtml . "\n    ", $htmlPos1, $htmlPos2 - $htmlPos1);
       }
    }
