<div class="text-center mb-4 border-b-2 border-slate-900 pb-3">
    ...
    <div class="text-xs font-bold text-slate-700 uppercase tracking-wider mt-0.5">
        Department of {{ $fullDepartment ?? $classroom->branch ?? 'Engineering' }}
    </div>
    <div class="text-[11px] text-slate-600 font-semibold mt-0.5">
        State Board of Technical Education (SBTE) Kerala — Revision 2021 Regulation (Clause 11.2.6)
    </div>
    <div class="mt-2">
        <span class="inline-block px-4 py-1 bg-slate-200 text-slate-900 font-extrabold text-xs uppercase tracking-wider rounded">
            {{ $reportTitle ?? 'Consolidated Seminar Evaluation Register' }}
        </span>
    </div>
</div>
...
<div class="w-full border border-slate-300 rounded-lg p-3 mb-4 bg-slate-50/60 text-xs grid grid-cols-2 md:grid-cols-4 gap-3">
    <div>
        <span class="text-[10px] text-slate-500 uppercase block font-semibold">Course Code &amp; Title</span>
    ...
    <div>
        <span class="text-[10px] text-slate-500 uppercase block font-semibold">Statutory Assessment Scheme</span>
        <span class="font-bold text-slate-900">CIA: 75 Marks (100% CIE)</span>
    </div>
</div>
