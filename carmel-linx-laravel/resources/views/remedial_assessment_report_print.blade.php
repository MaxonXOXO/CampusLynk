...
<head>
    ...
    <title>Remedial Assessment Report - {{ $assessment->title }}</title>
    <style>
        :root {
            --primary: #6b21a8;
            --border: #cbd5e1;
...
<body>
    ...
    <div class="a4-container">
        <div class="header">
            ...
            <h3>Remedial Sessions - Assessment Mark List</h3>
        </div>
        ...
        <div class="details-grid">
            ...
            <div class="details-item">
                ...
                <span class="details-val">{{ $subject ? $subject->subject_name : $room->subject_code }} ({{ $room->subject_code }})</span>
            </div>
            <div class="details-item">
                <span class="details-label">Assessment Title</span>
                <span class="details-val">{{ $assessment->title }}</span>
            </div>
            <div class="details-item">
                <span class="details-label">Assessment Type</span>
                <span class="details-val">{{ $assessment->type }} (Max Marks: {{ $assessment->max_marks }})</span>
            </div>
        </div>

        <div class="section-title">Consolidated Score Board</div>
        <table>
            <thead>
                <tr>
                    ...
                    @if($assessment->co_structure)
                        @foreach(array_keys($assessment->co_structure) as $co)
                            <th style="text-align: center; width: 80px;">{{ $co }} ({{ $assessment->co_structure[$co] }})</th>
                        @endforeach
                    @endif
                    <th style="text-align: right; width: 120px;">Total Score</th>
                </tr>
            </thead>
            <tbody>
                @forelse($students as $idx => $s)
                    <tr>
                        ...
                        @if($assessment->co_structure)
                            @foreach(array_keys($assessment->co_structure) as $co)
                                <td style="text-align: center; font-weight: bold;">{{ isset($s->co_scores[$co]) ? $s->co_scores[$co] : '-' }}</td>
                            @endforeach
                        @endif
                        <td style="text-align: right; font-weight: bold; color: var(--primary);">{{ $s->score }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" style="text-align: center; color: var(--text-muted); padding: 20px;">No student records found in this room.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
