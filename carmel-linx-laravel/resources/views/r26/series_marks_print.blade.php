...
<head>
    ...
    <title>Series Examination Marks Report - {{ $batchSubject->subject_name }}</title>
    <style>
        * {
...
<body>
    ...
    <div class="a4-page">
        ...
        <div class="header">
            ...
            <h3>Revision 2026 Scheme - Continuous Assessment Roster</h3>
        </div>
        ...
        <div class="report-title">
            Series Examination Marks Report
        </div>
                ...
                <tr>
                    ...
                    @foreach($seriesExams as $exam)
                        <th class="text-center" style="width: 15%;">
                            {{ $exam->exam_name }}<br>
                            <span style="font-size: 9px; font-weight: normal;">({{ implode('+', $exam->co_tags) }}) - {{ $exam->max_marks }}M</span>
                        </th>
                    @endforeach
                    <th class="text-center font-bold" style="width: 15%;">Consolidated<br><span style="font-size: 9px; font-weight: normal;">Scaled to 20M</span></th>
                </tr>
            </thead>
            <tbody>
                @forelse($studentCiaData as $sc)
                    <tr>
                        ...
                        @foreach($seriesExams as $exam)
                            <td class="text-center font-mono">{{ number_format($sc['exam_marks'][$exam->id] ?? 0.0, 1) }}</td>
                        @endforeach
                        <td class="text-center font-mono font-bold">{{ number_format($sc['series_exam_marks'] ?? 0.0, 1) }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="{{ 4 + count($seriesExams) }}" class="text-center italic">No student marks records found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
