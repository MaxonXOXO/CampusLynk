{{-- Tab: Program Outcomes (PO1 to PO11) Attainment Analysis --}}
<div class="space-y-4">
    <div class="flex items-center justify-between">
        <div>
            <h3 class="text-sm font-bold text-white">NBA Program Outcomes (PO1 – PO11) Evaluation</h3>
            <p class="text-xs text-slate-400">NBA Criterion 3 Attainment = (80% × Direct Course Attainment) + (20% × Indirect Surveys).</p>
        </div>
    </div>
                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <div class="space-y-1 max-w-xl">
                        <div class="flex items-center gap-2">
                            <span class="rounded-lg bg-indigo-500/20 px-2 py-0.5 text-xs font-bold text-indigo-400 font-mono">{{ $key }}</span>
                            <h4 class="text-xs font-bold text-white">{{ $po['title'] }}</h4>
                        </div>
                        <p class="text-[11px] text-slate-400 leading-relaxed">{{ $po['desc'] }}</p>
                    </div>
                    ...
                    <div class="flex items-center gap-4 bg-slate-950/40 rounded-xl p-3 border border-white/5">
                        <div>
                            ...
                            <input type="number" step="0.1" min="1.0" max="3.0" name="po_targets[{{ $key }}]" value="{{ number_format($target, 1) }}" class="po-target-input w-16 rounded border border-white/10 bg-slate-900 px-1.5 py-0.5 text-xs font-mono text-white text-center focus:border-indigo-500 focus:outline-none">
                        </div>
                        <div>
