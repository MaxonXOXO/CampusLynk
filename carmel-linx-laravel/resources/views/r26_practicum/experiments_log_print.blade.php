<x-layouts.report-layout 
    title="Practical Experiments Conducted & Attendance Log — {{ $batchSubject->subject_name }}"
    documentNo="DOC-R26-P-EXP-LOG"
...
>
    ...
    <div class="text-center border-b-2 border-slate-800 pb-3 mb-4">
        ...
        <h2 class="text-sm font-semibold text-slate-700 mt-1">Practical Experiments Conducted & Attendance Log — Revision 2026 Practicum</h2>
    ...
    <div class="grid grid-cols-3 gap-3 bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs mb-4">
        ...
        <div>
            <span class="text-slate-500 block font-medium">Total Lab Sessions Logged:</span>
            <span class="font-bold text-slate-900">{{ count($logs) }} Sessions</span>
        </div>
        <div>
            <span class="text-slate-500 block font-medium">Average Attendance:</span>
            <span class="font-bold {{ $avgAttnPct >= 75 ? 'text-emerald-700' : 'text-rose-700' }}">{{ $avgAttnPct }}%</span>
        </div>
        <div>
    ...
    <!-- Practical Experiments Conducted Table -->
        ...
        <table class="w-full text-left border-collapse text-[11px]">
            <thead>
                <tr class="bg-slate-800 text-white font-semibold">
                    ...
                    <th class="border border-slate-700 px-2 py-1.5 w-16 text-center">Hour</th>
                    <th class="border border-slate-700 px-2 py-1.5 w-16 text-center">Sub-Batch</th>
                    <th class="border border-slate-700 px-2 py-1.5">Practical Experiment / Topic Covered</th>
                    <th class="border border-slate-700 px-2 py-1.5 text-center w-14">Attended</th>
            ...
            <tbody>
                ...
                <tr class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-slate-50' }} hover:bg-slate-100/80">
                    <td class="border border-slate-200 px-2 py-1 text-center font-medium text-slate-600">{{ $log->sl_no ?? ($index + 1) }}</td>
                    <td class="border border-slate-200 px-2 py-1 text-center whitespace-nowrap">{{ $log->formatted_date }}</td>
                    <td class="border border-slate-200 px-2 py-1 text-center">{{ $log->period_label }}</td>
                    <td class="border border-slate-200 px-2 py-1 text-center font-mono">{{ $log->sub_batch }}</td>
                ...
                @empty
                <tr>
                    <td colspan="8" class="border border-slate-200 px-4 py-8 text-center text-slate-400 italic">
                        No practical experiment sessions or attendance logs recorded yet.
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="grid grid-cols-3 gap-8 mt-12 pt-4 border-t border-slate-300 text-center text-xs">
        <div>
            ...
            <p class="text-slate-500 text-[10px]">Lecturer / Lab Assessor</p>
        </div>
        <div>
            <div class="border-b border-slate-400 h-10 mb-2"></div>
