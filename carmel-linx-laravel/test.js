...
function switchStudentMentoringTab(tabId) {
  ...
  function renderDiaryAcademicProgress() {
    const container = document.getElementById('smdAcademicReport');
    if (!container) return;

    // Use the global academicData that is already loaded perfectly by the Academic Stats tab
    const statsData = window.academicData; 
    
    if (!statsData || !statsData.semesters || statsData.semesters.length === 0) {
    ...
    const cgpa = overall.cgpa || '-';
    const activityPoints = overall.activity_points || '-';
    ...
    let html = `
      <div class="grid grid-cols-2 gap-4 mb-5">
        <div class="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4 flex flex-col items-center justify-center shadow-inner">
          ...
          <span class="text-2xl font-black text-white">${cgpa}</span>
        </div>
        <div class="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4 flex flex-col items-center justify-center shadow-inner">
          <span class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Activity Points</span>
          <span class="text-2xl font-black text-amber-400">${activityPoints}</span>
        </div>
      </div>
    ...
    statsData.semesters.forEach(semData => {
      const sem = semData.semester;
      const sgpa = semData.sgpa || '-';
      const pts = semData.activity_points || '-';
      
      let rows = '';
      if (semData.subjects && semData.subjects.length > 0) {
        semData.subjects.forEach(sub => {
           // Display attendance as the primary indicator inside this consolidated view
           const att = sub.attendance_percentage !== null ? sub.attendance_percentage + '%' : '-';
           const attColor = (sub.attendance_percentage !== null && sub.attendance_percentage < 75) ? 'text-rose-400 font-black' : 'text-emerald-400 font-bold';
          ...
          <div class="flex justify-between items-end mb-2 px-1">
            ...
            <div class="flex gap-2">
              <div class="bg-slate-900 border border-slate-800 px-2 py-1 rounded flex items-center gap-1.5 shadow-inner">
                ...
                <span class="text-[11px] font-black text-white">${sgpa}</span>
              </div>
              <div class="bg-amber-950/20 border border-amber-900/40 px-2 py-1 rounded flex items-center gap-1.5 shadow-inner">
                ...
                <span class="text-[11px] font-black text-amber-400">${pts}</span>
              </div>
            </div>
          </div>
    ...
    setTimeout(() => {
      ...
      if (cvs) {
        ...
        const labels = statsData.semesters.map(s => `S${s.semester}`);
        const cdata = statsData.semesters.map(s => s.sgpa || 0);
