...
<body class="h-screen flex flex-col bg-[#0b1121] overflow-hidden">
    ...
    <nav class="h-16 border-b border-slate-800/60 bg-slate-900/60 backdrop-blur-md flex items-center justify-between px-6 z-10 shrink-0">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-lg font-extrabold text-slate-100 tracking-tight flex items-center gap-2">
        ...
        <div class="flex items-center gap-3">
            ...
            <span class="font-bold text-sm text-slate-200 hidden sm:block">{{ session('userName') }}</span>
        </div>
    </nav>
    ...
    <main class="flex-1 overflow-y-auto p-6 md:p-8">
        <div class="max-w-6xl mx-auto space-y-6">
            ...
            <!-- BREADCRUMBS -->
            <div id="cfBreadcrumbs" class="hidden flex items-center gap-2 text-xs font-bold text-slate-400 bg-slate-900/50 p-3 rounded-xl border border-slate-800 shadow-sm">
            ...
            <div id="cfLevel1" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 animate-fade-in">
                <div class="col-span-full py-20 text-center text-slate-500 font-bold text-sm animate-pulse">Loading assigned courses...</div>
            </div>
            ...
            <div id="cfLevel4" class="hidden space-y-6 animate-fade-in">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm">
                    <div>
                        <h2 id="cfSubjectTitle" class="text-2xl font-black text-white mb-1">Subject Name</h2>
                        <p id="cfBatchInfo" class="text-xs text-slate-400 font-medium">Batch Info</p>
                    </div>
                    <div class="flex items-center gap-3">
                        <span id="cfStatusBadge" class="px-3 py-1 rounded-lg border text-xs font-bold uppercase tracking-wider text-amber-400 bg-amber-500/10 border-amber-500/20">DRAFT</span>
                        <button type="button" onclick="saveCourseFileProgress()" class="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-premium shadow-lg shadow-indigo-500/20">Save Draft</button>
                        <button type="button" onclick="generateCourseFilePDF()" class="bg-amber-500 hover:bg-amber-400 text-slate-950 px-5 py-2.5 rounded-xl text-sm font-black transition-premium flex items-center gap-2 shadow-lg shadow-amber-500/20"><span class="material-symbols-rounded text-lg">picture_as_pdf</span> Generate PDF</button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div class="lg:col-span-3 space-y-2">
                        <button type="button" onclick="switchCfTab('A')" id="tabBtnA" class="w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-premium bg-amber-500/10 text-amber-400 border-l-4 border-amber-500">Section A: Planning</button>
                        <button type="button" onclick="switchCfTab('B')" id="tabBtnB" class="w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-premium text-slate-400 hover:bg-slate-800 hover:text-white border-l-4 border-transparent">Section B: Materials</button>
                        <button type="button" onclick="switchCfTab('C')" id="tabBtnC" class="w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-premium text-slate-400 hover:bg-slate-800 hover:text-white border-l-4 border-transparent">Section C: Assessments</button>
                        <button type="button" onclick="switchCfTab('D')" id="tabBtnD" class="w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-premium text-slate-400 hover:bg-slate-800 hover:text-white border-l-4 border-transparent">Section D: Attainment</button>
                    ...
                    <div class="lg:col-span-9 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm min-h-[400px]">
                        <form id="cfForm">
                            ...
                            <div id="cfTabA" class="space-y-6 animate-fade-in">
                                <div class="bg-slate-950/50 border border-slate-800/60 p-4 rounded-xl flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-slate-400 leading-relaxed">Syllabus, Course Outcomes (COs), and your detailed Lesson Plans will be automatically injected into the final PDF. You do not need to re-enter them here.</p>
                                    </div>
                                </div>
                                <div class="space-y-4">
                                    <div>
                                        <label class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-2">Gaps Identified (if any)</label>
                                    ...
                                    <div>
                                        <label class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-2">Bridge Topics Delivered</label>
                            ...
                            <div id="cfTabB" class="hidden space-y-6 animate-fade-in">
                                <div>
                                    <label class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-2">NPTEL / Swayam Links</label>
                                ...
                                <div>
                                    <label class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-2">Other Reference Materials / Handouts</label>
                            ...
                            <div id="cfTabC" class="hidden space-y-6 animate-fade-in">
                                <div class="bg-slate-950/50 border border-slate-800/60 p-4 rounded-xl flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-slate-400 leading-relaxed">Test Questions, Assignment Questions, and the final Student Mark Sheets will be attached automatically.</p>
                                    </div>
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-2">Evaluation Scheme</label>
                            ...
                            <div id="cfTabD" class="hidden space-y-6 animate-fade-in">
                                <div>
                                    <label class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-2">Action Taken Report (if COs not fully attained)</label>
                                ...
                                <div>
                                    <label class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-2">Course Committee Minutes (Summary)</label>
    ...
    <!-- Global Toast -->
    <div id="globalToast" class="fixed bottom-6 right-6 px-6 py-4 rounded-2xl font-bold text-sm shadow-2xl transition-all duration-300 translate-y-24 opacity-0 z-50 flex items-center gap-3"></div>
    ...
    <script>
        ...
        function renderChecklist(savedDocs) {
            ...
            defaultChecklist.forEach((name, idx) => {
                ...
                let actionBtn = '';
                const autoGen = [3, 4, 6, 8, 11, 21, 22];
