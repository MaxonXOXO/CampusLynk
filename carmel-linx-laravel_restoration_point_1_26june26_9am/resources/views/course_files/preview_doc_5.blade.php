<div class="w-full h-full min-h-[700px] flex flex-col">
    @if($pdfUrl)
        <div class="mb-4 flex justify-between items-center">
            <h3 class="text-xl font-bold text-slate-800">Course Information Sheet (CIS)</h3>
            <div class="flex items-center gap-3">
                <button onclick="document.getElementById('cisPdfInput').click()" class="px-4 py-2 bg-slate-100 text-slate-700 font-bold text-sm rounded-lg hover:bg-slate-200 transition-colors flex items-center gap-2">
    ...
    @else
        <div class="flex-1 flex flex-col items-center justify-center text-center p-10 bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl">
            <div class="w-20 h-20 bg-indigo-50 text-indigo-400 rounded-full flex items-center justify-center mb-4">
                <span class="material-symbols-rounded text-4xl">upload_file</span>
            </div>
            <h3 class="text-xl font-black text-slate-800 mb-2">Upload Course Information Sheet</h3>
            ...
            <div id="uploadStatus" class="mt-4 text-sm font-bold text-emerald-600 hidden">Uploading... Please wait.</div>
        </div>
    @endif
</div>
