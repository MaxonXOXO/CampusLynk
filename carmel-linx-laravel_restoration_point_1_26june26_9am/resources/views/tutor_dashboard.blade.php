...
<body class="bg-slate-900 text-slate-100 min-h-screen flex flex-col md:flex-row overflow-x-hidden">
  ...
  <aside class="w-full md:w-64 bg-slate-950 text-white flex-shrink-0 flex flex-col border-r border-slate-800/80 z-20 shadow-xl md:sticky md:top-0 md:h-screen">
    ...
    <nav class="flex-grow p-4 space-y-1.5">
      ...
      <button id="navActivity" onclick="switchPanel('activity')" class="w-full text-left px-4 py-2.5 rounded-xl font-bold text-xs flex items-center gap-3 transition-premium text-slate-400 hover:bg-slate-800 hover:text-white cursor-pointer">
  ...
  <main class="flex-grow flex flex-col relative min-h-screen">
    ...
    <div class="flex-grow overflow-y-auto p-6 md:p-8 space-y-6">
      ...
      <div id="panelRoster" class="space-y-6">
        ...
        <div onclick="toggleRoster()" class="flex justify-between items-center bg-slate-950/30 hover:bg-slate-900/60 border border-slate-800/40 p-4 rounded-2xl cursor-pointer transition-premium">
          ...
          <div class="flex items-center gap-2">
            <button onclick="event.stopPropagation(); promoteBatch()" class="px-4 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white rounded-xl text-xs font-bold transition-premium cursor-pointer flex items-center gap-1.5 shadow-lg shadow-emerald-500/10">
              <span class="material-symbols-rounded text-sm">upgrade</span> Promote Semester
            </button>
            <button onclick="event.stopPropagation(); openRegisterModal()" class="px-4 py-2.5 bg-gradient-to-r from-blue-500 to-sky-600 hover:from-blue-600 hover:to-sky-700 text-white rounded-xl text-xs font-bold transition-premium cursor-pointer flex items-center gap-1.5 shadow-lg shadow-blue-500/10">
          ...
          <div class="overflow-x-auto overflow-y-auto max-h-[500px] scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
            <table class="w-full text-left text-xs border-collapse relative">
              <thead class="sticky top-0 z-10 shadow-md">
                <tr class="bg-slate-900 border-b border-slate-800/60 text-slate-400 font-bold">
      ...
      <div id="panelActivity" class="hidden space-y-6">
        ...
        <!-- Activity Header -->
        <div onclick="toggleActivity()" class="flex justify-between items-center bg-slate-950/30 hover:bg-slate-900/60 border border-slate-800/40 p-4 rounded-2xl cursor-pointer transition-premium">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-xs font-black text-slate-200">Activity Points Verification</h3>
              <p class="text-[10px] text-slate-400 mt-0.5">Review and verify extracurricular claims submitted by students in your batch.</p>
            </div>
          </div>
          <button onclick="event.stopPropagation(); loadActivityClaims()" class="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded text-[10px] font-bold transition-premium flex items-center gap-1">
        ...
        <div id="activityContent" class="space-y-6 transition-all duration-300 origin-top" style="max-height: 0px; opacity: 0; overflow: hidden;">
          <div class="overflow-x-auto rounded-xl border border-slate-800/40">
            <table class="w-full text-left text-xs border-collapse whitespace-nowrap">
              <thead>
                <tr class="bg-slate-900 border-b border-slate-800/60 text-slate-400 font-bold uppercase tracking-wider text-[9px]">
                  <th class="p-3">Student</th>
  ...
  <script>
    ...
    function switchPanel(panelId) {
      ...
      const panels = ['roster', 'audit', 'profile', 'mentoring', 'activity'];
      panels.forEach(id => {
              ...
              if (c.status === 'Pending') {
                actionsHtml = `
                  <div class="flex items-center justify-center gap-2">
                    ...
                    <button onclick="verifyClaim('${c.id}', 'Verified')" class="px-2 py-1 bg-teal-600 hover:bg-teal-500 rounded text-white text-[10px] font-bold">Approve</button>
                    <button onclick="verifyClaim('${c.id}', 'Rejected')" class="px-2 py-1 bg-red-950 text-red-400 border border-red-900 rounded text-[10px] font-bold">Reject</button>
                  </div>
                `;
              } else {
                actionsHtml = `<span class="font-bold ${c.status === 'Verified' ? 'text-teal-400' : 'text-red-400'}">${c.status} (${c.points_awarded} pts)</span>`;
              }

              html += `
                <tr class="hover:bg-slate-900/50 transition-premium">
                  <td class="p-3">
                    <span class="font-bold text-slate-300 block text-[11px]">${c.student.name}</span>
                    <span class="text-[10px] text-slate-500 font-mono">${c.reg_no}</span>
                  </td>
                  <td class="p-3 text-[11px] font-bold text-slate-400">${c.activity_segment}</td>
                  <td class="p-3">
                    <span class="block text-[11px] text-slate-300">${c.activity_name}</span>
                    <span class="block text-[10px] text-slate-500">${c.level}</span>
                  </td>
                  <td class="p-3 text-[10px] text-slate-500 whitespace-normal min-w-[150px]">${c.document_reference || 'N/A'}</td>
                  <td class="p-3 text-center text-[11px] font-bold text-slate-300">${c.points_claimed}</td>
    ...
    function verifyClaim(id, status) {
      ...
      fetch(`/api/tutor/activity-points/${id}/verify`, {
        method: 'POST',
        headers: getHeaders(),
        body: JSON.stringify({ status: status, points_awarded: awarded })
      })
      .then(res => res.json())
      .then(data => {
        if (data.status === 'SUCCESS') {
